	...
	<body>
		...
		<div id="show-userInfo" class="content scaffold-show" role="main">
			...
			<ol class="property-list userInfo">
				...
				<g:if test="${userInfoInstance?.groups}">
				<li class="fieldcontain">
					<span id="groups-label" class="property-label"><g:message code="userInfo.groups.label" default="Groups" /></span>
					
						<g:each in="${userInfoInstance.groups}" var="g">
						<span class="property-value" aria-labelledby="groups-label"><g:link controller="userGroup" action="show" id="${g.id}">${g?.encodeAsHTML()}</g:link></span>
						</g:each>
